...
// assistant-ui 메시지 metadata에 넣을 스트리밍 시간 정보를 계산합니다.
// 실제 토큰 수가 없으므로 텍스트 길이 기반의 대략적인 tokenCount/tokensPerSecond를 기록합니다.
type TrackingState = {
  messageId: string;
  startTime: number;
...
export const useLangGraphStreamingTiming = (
  ...
  useEffect(() => {
    ...
    // 실행 중에는 마지막 assistant 메시지를 추적하고, 텍스트 길이가 늘어나는 순간을 chunk로 봅니다.
    if (isRunning && lastId) {
      ...
      // 실행이 끝나면 누적된 관측값을 고정된 timing metadata로 저장합니다.
      const t = trackRef.current;
