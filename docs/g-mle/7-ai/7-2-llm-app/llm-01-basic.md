...
- 인코더에서 나온 벡터를 컨텍스트 벡터라고 하며, 문맥을 담고있다.  

![Alt text](image-7.png)
- 1, 영어문장이 인코더로 들어가서 벡터로 변환이 된다. 이를 컨텍스트 벡터라고 함. (문맥 담고 있음)  
- 2, 인코더에서 나온 벡터를 디코더에서 참조를 하는데, 전문용어로 어텐션이라 함.   

![Alt text](image-13.png)  
- *어텐션은 2개 문장 사이에서 관계 점수를 구하는 것  
- 인코더에서는 문장과 문장의 사이의 관계를 구하기 위해서 셀프어텐션을 구한다.  


![Alt text](image-9.png)
- 셀프 어텐션은 같은 문장을 가지고 관계점수를 구하는 것, 이유는 문장 내 단어들 관계를 이해하기 위해
...
- 3,인코더-디코더 어텐션( 인코더의 출력 컨텍스트 벡터 - 디코더의 입력 간) 을 구함  

![Alt text](image-10.png)
- 위 1~3 과정을 거친다.  

트랜스포머를 이용해 GPT와 BERT가 나왔다. 2가지가 양대산맥이다.  

![Alt text](image-11.png)  
- 버트(구글)와 GPT(OpenAI)의 차이점이 중요.  
- 버트는 트랜스포머 모델 중 인코더를 쌓은 것, GPT는 디코더를 쌓아 만들었다.  
- 버트는 가벼운 대신에 도메인에 맞게 파인튜닝을 해야 한다. 반대로 GPT는 파인튜닝을 안하기 위해 모든 데이터를 학습했다.
...
- Embeddings, Re-Ranking  
  - *벡터 임베딩 : 빠르지만 비교적 낮은 유사도 판단, 임베딩된 쿼리와 임베딩된 문서를 벡터 유사도로 비교한다.  
  - *Reranking 모델 : 벡터 유사도 부분 마져도 작은 LLM(비유)이 유사도를 판단한다. ( 1~10억개 파라미터의 re-ranking model)   
