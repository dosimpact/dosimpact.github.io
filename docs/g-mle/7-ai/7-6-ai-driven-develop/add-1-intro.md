...
솔루션 : 조직 차원의 워크플로우 배포 플랫폼 
  - 조직이 일하는 방식을 워크플로우로 정의하고 이를 한 곳에서 배포&관리한다.  
...
- 인간이 걸림돌이며 이는 사람이 봐야 할 양이 너무 많고 머릿속 처리(내용의 재구조)의 한계에서 발생한다.  
  - 1, 양을 해결 하기 위해서는 상위 수준의 계획서, 상위 수준의 설계, 상위 수준의 검증 리포트가 필요하다.
  - 2, 머릿속 처리 가속을 위해서, 미리 사전에 정의한 프레임워크, 템플릿, Problem Solving 양식을 정의하는 게 필요하다.  
