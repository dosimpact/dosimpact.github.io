...
📌 slice  
- substring 대신 slice을 사용할 것  ( substring은 음수를 0으로 간주하여 처리한다)  
- substr은 비권장 (비일관성 인자)
```js
'abcd'.slice(1) // bcd
'abcd'.slice(-1) // d
...
📌 slice, splice, (rotate)
- *slice*(immutable)
- splice(mutable) : splice(시작 인덱스, 제거할 요수 수, 추가할 아이템 들)     
  - 제거를 먼저하고 추가 된다.
```js
// slice
const arr = ['a','b','c'];
...
```js
// 재귀 함수를 이용해 프리미티브 타입일 때까지 재귀함수를 호출한다.
...
- comparator 함수에서 -1 = a, b는 올바른 순서, 0 = 판단 불가, 1 = 역순이다.  
