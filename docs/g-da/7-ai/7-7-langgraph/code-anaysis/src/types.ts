...
// 이 패키지에서 다루는 LangChain/LangGraph 메시지와 assistant-ui 연결용 이벤트 타입 계약입니다.
// 런타임 코드는 이 타입들을 기준으로 스트림 이벤트를 병합하고 assistant-ui 메시지로 변환합니다.
...
export enum LangGraphKnownEventTypes {
  // LangGraph SDK streamMode에서 받을 수 있는 주요 이벤트 채널입니다.
  Messages = "messages",
...
export type LangChainMessage =
  // LangChain의 system/human/ai/tool 메시지를 assistant-ui 변환에 필요한 필드만 모델링합니다.
  | {
      id?: string;
      type: "system";
      content: string;
...
> = {
  // LangSmith typed_ui/push_ui_message가 보낸 Generative UI 조각입니다.
  type: "ui";
  id: string;
  name: TName;
  props: TProps;
