...
# 요구사항: Chapter 1: Prompt Chaining

## 출처

- PDF: `Agentic_Design_Patterns.pdf`
- 섹션: `Chapter 1: Prompt Chaining`
- 페이지 범위: `docs/agentic-design-patterns-toc.md` 기준 논리 페이지 `18-29`
- 추출 참고: 보이는 Chapter 1 제목은 PDF 페이지 라벨 `23` / 0 기반 인덱스 `22`에서 발견되었고, Chapter 2는 PDF 페이지 라벨 `36` / 0 기반 인덱스 `35`에서 시작한다. 이 요구사항은 위 TOC 범위를 인용용 논리 범위로 유지하면서, 추출에는 보이는 장 경계 페이지 `23-35`를 사용한다.

## 패턴 요약

프롬프트 체이닝은 복잡한 LLM 작업을 순서가 있는 집중된 하위 작업으로 분해한다. 
각 단계는 좁은 책임을 가지며, 한 단계의 출력이 다음 단계의 입력이 된다. 이를 통해 지시사항 과부하, 컨텍스트 이탈, 환각 위험, 디버깅하기 어려운 거대한 단일 프롬프트를 줄인다.

작업에 여러 의존 단계가 있거나, 중간 검증이 필요하거나, 구조화된 전달 데이터가 유리하거나, LLM 호출 사이에 결정론적 처리 또는 외부 도구 호출이 필요한 경우 이 패턴을 사용한다. 
이 장은 JSON 같은 구조화된 중간 출력을 강조한다. 모호한 자연어 전달은 이후 단계의 실패를 일으킬 수 있기 때문이다.

구현 관점에서 프롬프트 체이닝은 상태를 가진 파이프라인처럼 동작해야 한다. 중간 산출물을 보존하고, 각 전달 지점을 검증하며, 가능한 경우 잘못된 출력을 재시도하거나 복구하고, 디버깅에 충분한 상태를 노출해야 한다.
...
- `docs/agentic-design-patterns-toc.md`는 Chapter 1을 논리 페이지 `18-29`로 나열하지만, PDF 텍스트 추출에서는 보이는 Chapter 1 섹션이 페이지 라벨 `23-35`에 있다. 향후 요구사항 문서에서 TOC 논리 페이지만 인용할지, 추출된 PDF 라벨만 인용할지, 둘 다 인용할지 확인이 필요하다.
