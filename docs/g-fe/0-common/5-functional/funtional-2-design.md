...
- 원칙 2, 코드가 읽기 어려운 이유는 여러 계층이 섞여있어서 그렇다. 아래의 예시의 계층을 보자.  
  ...
  - * 동일한 계층의 함수들은 하위 계층의 함수들을 이용해 구현한다. 동일 계층을 서로 참조하지 않는다. 여러 수준의 함수로 구현되었다면 코드스멜이다.  
...
계층형 설계의 4가지 패턴   
1, 직접 설계 : 하위 계층을 이용해 함수를 구현하는 것
  - 만약 너무 깊은 계층을 사용해서 구현을 하고 있으면 코드 스멜을 의심한다. (호출 그래프의 화살표의 길이가 긴 경우 )  
  - 즉, 동일한 계층은 하위 동일한 Level의 계층을 이용해 만드는 게 좋다. 


2, 추상화 벽 : Backend와 API 인터페이스를 미리 정하는 것과 비슷하다.  
...
3, 작은 인터페이스 : 시스템이 커질수록 작고 강력한 기능의 인터페이스로 만드는 게 좋다.   
