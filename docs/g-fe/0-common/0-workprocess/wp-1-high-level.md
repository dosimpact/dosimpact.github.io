...
📌 필수 단위 테스트 코드 작성
- [ ] 그래프, 수치에 대한 함수는 테스트 코드를 반드시 작성한다. 예를 들어 계산식, 선형보간법, 특정 공식 구현, 주요 비즈니스 로직  
...
- PageSize가 10일 때는 50Page가 있지만, PageSize가 50일 때는 50번째 페이지는 없다. 적절하게 마지막 페이지로 이동해야 함.  
- 메인 필터에 따라서, 서브 필터에 상품군의 개수가 달라진다. 모수가 바뀌는 것 대응    
