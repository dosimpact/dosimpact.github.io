...
일치하는 문자열의 index를 찾을 때 유용하게 사용된다.    
- 1.g플래그 없다면, 매칭되는 첫번째 문자만 리턴한다.
  - ( Named groups 을 사용할 때는 groups속성을 리턴해야 하므로 g플래그를 사용하면 안 된다.  )
...
- 그래서 일부 문자는 4바이트로 인코딩되어 있습니다. 예를 들면 𝒳(수학에서 사용하는 X)나 😄(웃는 표정), 일부 상형 문자  
...
>\n 개행을 기준으로 여러행들을 모두 검사하고 싶다면 m(멀티라인) 플래그 사용할 것.  
...
```[ \ ^ $ . | ? * + ( ).```      
- 위 특별한 문자열들을 정규식의 문법으로 사용하고있지만, 특수문자들 자체를 고를 때는 이스케이핑(문법적 의미 탈출)을 해야 한다.  
...
오답노트 : {6} 이라고쓰면 6개 문자열만 선택할 것 같지만 선택은 그렇게 되지만, 실제로는 #11111123 등의 문자열에서도 선택된다.  
- 조건을 걸어서 단어를 끊어야 한다.  
...
2.그 이후 .+ 을 만났으니 모든 문자열을 Grab 한다.  

3.너무 많이 Grab한 것 같아 다시 뒤로 빽 한다.  
'a "witch" and her "broom" is one'  
   ^^^^^^^^^^^^^^^^^^^^^^^ "라는 패턴이 나올 때까지 빽 한다.  

우선 수량 연산자를 만나면 Grab하고 그 후 패턴이 나오면 빽 하는 로직이다.  
```
...
- .으로 광범위한 단어 및 수량자로 패턴을 만드는 것이 모든 해결책은 아니다.  
...
- `(?<name>)` 이러한 패턴으로 사용한다.
  - *g태그를 사용하면 안 된다. g는 match 리턴을 배열로 처리한다.*
...
- 2.수량자를 확인한다.  
- 3.정규식의 grab부분 이후에는 condition문법을 해석한다.  
...
문자열을 grab하지 않고 특정 패턴과 전방 일치 혹은 후방 일치 하는 조건을 걸고싶을 때 사용 한다.  
...
```js
// 1개이상의 숫자덩어리를 찾을꺼야, 단 €가 있어야 하며 이를 선택하지 말 것 
...
lookahead를 조건으로 사용도 하면서, grab도 하고 싶은 경우에는 괄호를 감싸준다.  
- 대신 별도의 grab으로 도출된다.  
