...
- [React 공식문서](#react-공식문서)
  ...
  - [3.4 State를 보존하고 초기화하기 : 리액트 상태 렌더 트리의 위치(자리)가 중요하다.](#34-state를-보존하고-초기화하기--리액트-상태-렌더-트리의-위치자리가-중요하다)
...
- 단, 혹시라도 자식 컴포넌트 중 state가 관리된다면 예기치 못한 버그가 생길 것이다.  



## 3.State 관리하기

## 3.4 State를 보존하고 초기화하기 : 리액트 상태 렌더 트리의 위치(자리)가 중요하다.  
...
3.같은 위치의 다른 key를 가진 컴포넌트는 state를 초기화 한다.  
  - props가 변경되는 것은 리렌더링의 대상이지, 컴포넌트 재생성을 위해서는 key를 변경해야 한다.  
...
- Get Input Value  : ref를 이용해 입력값들을 수집한다.   
