...
- 기존 방식은 모든 소스 코드를 바탕으로 번들 파일을 준비해두었다면, Native ESM은 필요한 것만 그때그때 제공    

2.HMR(Hot Module Replacement)  
- 모든 파일에 대해서 번들링을 하는 것은 비효율적이다. HMR 이라는 대안이 나왔다.  
- vite도 HMR 지원, 그 과정에서 ESM을 이용해 앱 사이즈와 무관함.  
