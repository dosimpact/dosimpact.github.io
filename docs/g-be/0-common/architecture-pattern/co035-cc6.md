...
- [Cloud SW 아키텍처 패턴:Reliability, Error Handling, Recovery Patterns](#cloud-sw-아키텍처-패턴reliability-error-handling-recovery-patterns)
  ...
  - [Throttling and Rate Limiting Pattern](#throttling-and-rate-limiting-pattern)
    ...
    - [Client Side throttling](#client-side-throttling)
      - [여러 가지 throttling 전략들을 구사할 수 있다.](#여러-가지-throttling-전략들을-구사할-수-있다)
      - [여러 가지 Global Rate Limit 정책](#여러-가지-global-rate-limit-정책)
  - [Retry Pattern](#retry-pattern)
    - [문제 정의](#문제-정의)
    - [해결](#해결-1)
    - [에러 카테고리화 - User Error vs System Error](#에러-카테고리화---user-error-vs-system-error)
    - [고려점](#고려점)
  - [Circuit Breaker Pattern](#circit-breaker-pattern)
    - [문제점](#문제점)
    - [해결법](#해결법)
    - [half-open](#half-open)
    - [고려점](#고려점-1)
...
- 서버의 리소스 한계로 서버가 죽을 수 있다. 이는 서비스 수준 협약 SLA 위반  
...
해결 방법은 적절한 Throttling and Rate Limiting 도입  
...
#### 여러 가지 throttling 전략들을 구사할 수 있다.   


![](./img/co47.png)

1.Dropping Request : 429 Too Many Request 요청으로 응답한다.   
...
### 에러 카테고리화 - User Error vs System Error 

사용자 에러인가 ? 시스템 에러인가 ? 구분

사용자 에러  
- 403 Not Authorized 에러는 사용자의 권한이 없다는 것이다.  
- 이는 사용자가 발생시킨 정상적인 오류이므로, 적절한 피드백이 제공     

시스템 에러
- 500 Internal Server Error는 시스템 내부의 핸들링 못한 오류로 발생    
- 사용자에게 오류정보는 드러내면 안 된다.   


### 고려점

1.어떤 오류일 때 재시도할지 선택  
- 503 Service Unavailable 에러라면 성공할 때 까지 몇 번 더 요청할 수 있다.  
...
4.재시도 횟수와 시간  
- 예) 5분간 5xx에러가 200개가 넘으면 장애 상황이다.   
- 클라이언트는 더이상 요청을 하지 않는다.  
- 관련 부서에 에러를 알려야 한다. on-call
...
- 이런 경우 다시 retry 하는데, 중복결제가 발생하면 안 된다.   
...
- 에러 비율이 높아지는 순간, 더이상 외부 서버에 요청을 보내지 않기 위해 회로를 Open 한다.  
...
- 보내는 방법은 2가지 이
- 데드 레터 큐는 시스템이 처리하지 못한 에러메시지들이 쌓여있다.  
...
Dead Letter 로 보낼 때 적절한 에러 메시지나 코드 등을 넣는 것이 중요하다.  
