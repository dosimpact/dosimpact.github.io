...
- 2가지 입력값을 특정 키 기준으로 비교한다.  
  - A만 있는거, B만 있는거, 같은거, 일부만 다른 것 4가지 출력이 나옴.  
  - 이렇게 복잡한 케이스를 처리해야 할 때가 있나?  

## C3. Looping

📕 언제 사용?
루프 노드는 데이터 스트림에서 Item 단위로 처리할 때 유용하다.  
- 예를 들어 배열지원이 없느 HTTP Request를 보내는 경우,  
...
- Loop 만드는 방법. 
  - 1) IF문으로 조건이 충족될 때까지 무한루프 고리 만들기.  
  - 2) Loop 노드 사용.  
...
- 워크플로우를 많이 만들면 논리적으로 동시에 실행되는 것 처럼 보이지만 순서가 있다. 
...
📕 에러 핸들링  
워크플로우를 작성하다보면 에러를 다루어야 한다. 여기서 에러 핸들링은 좀 다른 개념이다.  
- 예를 들어 http요청 결과 4xx, 5xx 에러가 발생해서 노드가 실패할 수 있다.  
- n8n워크플로우 입장에서는 이것을 워크플로우 자체의 실패로 판정할지, IF문을 이용해 적절한 조치를 할지는 사용자에게 달려 있다.  
- Case 1. 만약에 반드시 성공해야 할 http 요청이였다면(그래야 워크플로우가 의미가 있는 경우) 워크플로우 자체의 에러를 던지는 것도 좋다.  
...
- 위 사진처럼 워크플로우 세팅에서 에러 트리거 워크플로우를 설정해야 한다.  
- Webhook 노드를 만들어서 Production 모드로 테스트를 진행하면, 아래와 같은 결과.  

![Alt text](image-4.png)

사용 노드 정리  
- Stop and Error 노드 : 에러 발생시킴
  - 에러 메시지 전달 가능.  
- Error Trigger 노드 : 워크플로우의 설정에 연결된 에러트리거를 받는 노드  
