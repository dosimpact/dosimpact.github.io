...
1-1, 인코더/디코더 
- 비유 : 주스 만들기 
- 인코더는 과일을 갈아서 농축액을 만드는 역할, 디코더는 농축액에서 물을 섞어 주스로 만드는 역할.  
...
3-1, 어텐션 매커니즘, K, V, Query Vector 설명  

Q, K, V
- Query (Q): 내가 검색창에 친 '검색어' 내가 지금 찾고 싶어 하는 정보의 주체입니다. (예: "달콤한 과일")  
- Key (K): 책장에 꽂힌 책들의 '제목(라벨)' 도서관에 있는 수많은 정보가 각각 어떤 주제인지 알려주는 이름표입니다. (예: "사과", "고구마", "망치")  
- Value (V): 그 책 안에 담긴 '실제 내용' 제목을 보고 책을 펼쳤을 때 얻을 수 있는 진짜 정보입니다. (예: "빨갛고 아삭한 식감...", "달고 부드러운 맛...")  

3-2, 어텐션이 계산되는 3단계  

- 1, 비교하기 (Q × K): 내가 입력한 검색어(Q)와 책장의 제목들(K)을 하나씩 대조해 봅니다.
  - "달콤한 과일"과 "사과"는 비슷하네? (높은 점수)
  - "달콤한 과일"과 "망치"는 전혀 상관없네? (0점)
- 2, 점수 매기기 (Softmax): 어떤 책이 가장 내 검색어와 관련 있는지 비율(%)을 정합니다.
  - 사과 70%, 고구마 28%, 망치 2%... 이런 식으로요.
- 3, 가져오기 (Score × V): 이 비율을 실제 내용(V)에 곱해서 가져옵니다.
  - 관련성 높은 '사과'의 내용은 많이 가져오고, '망치'의 내용은 거의 무시합니다.
  - 이것들을 다 합치면 **"내가 찾던 정보의 최종 요약본"**이 완성됩니다.   


3-3, 단어 임베딩 (Word Embedding) 혹은 시맨틱 공간(Semantic Space)  
