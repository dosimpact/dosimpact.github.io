...
1.특정 네트워크 종속성  
- L2솔루션을 위해 아비트럼을 사용한다면, 당연히 아비트럼 네트워크 안에서 트랜잭션이 발생해야 한다.  
...
솔라나는 초당 수천건의 트랜잭션을 처리하도록 고성능 블록체인으로 설계되었다.  
- 고성능 스포츠카는 빠르게 달릴 수 있지만, 엔진과열이 된다.  
- 솔라나도 트래픽 급증이 되면 네트워크가 과부하 되어 트랜잭션이 지연되거나 실패한다.  
...
- 수수료가 줄어들어 트랜잭션이 늘어나면 괜찮지만, 수익을 보상해 줄만큼 늘지 않았다.  

L2 토큰들과 L1 토큰들의 상호작용이 좋아야 한다.  
- Stage1의 L2 토큰만 인정을 할 것이다 라는 방향  
