...
Contract Storage. 
- 저수준의 언어는 stack을 이용해 함수에 값을 전달한다.  
- EVM은 256bit(32byte) 레지스터 스택을 사용한다. 스택은 총 한 번에 1024 아이템만 접근 및 조작 가능.   
- 레지스터 스택은 연산을 위한 임시 데이터 저장소라고 생각

- 이러한 레지스터의 한계로 contract storage, memory를 사용한다. memory, storage 키워드를 사용하여 데이터를 저장.  
- Contract Storage는 마치 공공 DB처럼 작동, 계약에 트랜잭션을 보내지 않아도 값을 읽을 수 있다.  
  - Storage에 값을 쓰는건 매우 비싸다. 6천배나  
  - Memory는 덜 하다.  
- Contract Memory는 계약 실행 중 임시데이터 저장소 이다.    
...
- *블록에는 여러 트랜잭션이 포함되며, 최초 블록 생성자에게 보상을 준다.  
