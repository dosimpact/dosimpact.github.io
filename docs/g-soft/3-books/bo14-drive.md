...
- 악셀을 밟으면서 회전하는 것은 최대한 지양 해야함.  
- 속도를 내서 직진하다가 핸들링할 때 반브레이크나, 노악셀로 핸들링 하기.     
...
- 2. 앞의 차량만 브레이크면 > 경종  
- 3. 앞의 모든 차량이 브레이크를 한 번에 밟는다? -> 위험, 비상깜빡이 키고 브레이크 밟아야 할 수도.  
...
- 100km를 맞추는 것도 중요하지만, 앞차와 계속 거리가 멀어지면 중간에 차가 계속 들어온다.  
- 그래서 앞차와의 페이스를 맞추는 것.  
